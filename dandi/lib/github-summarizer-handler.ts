...
import { apiKeyOwnedByUser, secretExists } from "@/lib/api-keys-db";
...
export function extractGithubSummarizerSecret(request: Request, bodySecret: string | undefined): string {
...
export type GithubSummarizerPostBody = {
  secret?: string;
  githubUrl?: string;
};

/** Parses JSON body on POST when `Content-Type` includes `application/json`. */
export async function tryParseGithubSummarizerPostBody(
  request: Request,
): Promise<GithubSummarizerPostBody | null> {
...
export type HandleGithubSummarizerOptions = {
  /** When set, the secret must belong to this OAuth user before quota is claimed. */
  restrictToUserId?: string;
};

async function assertSecretAllowed(secret: string, restrictToUserId?: string): Promise<NextResponse | null> {
  if (!restrictToUserId) return null;
  const owned = await apiKeyOwnedByUser(restrictToUserId, secret);
  if (!owned) {
...
async function runSummarizerForGithubUrl(githubUrl: string): Promise<NextResponse> {
  try {
...
export async function handleGithubSummarizer(
  request: Request,
  options?: HandleGithubSummarizerOptions,
): Promise<NextResponse> {
  const post = await tryParseGithubSummarizerPostBody(request);
  const bodySecret = post?.secret;
  const secret = extractGithubSummarizerSecret(request, bodySecret);
  ...
  try {
    ...
    if (request.method === "POST" && githubUrl) {
      const denied = await assertSecretAllowed(secret, options?.restrictToUserId);
      ...
      return runSummarizerForGithubUrl(githubUrl);
    }
